...
export default function ProjectInformationPage() {
    ...
    const handleDownloadPdf = () => {
        ...
        const addField = (label: string, value: string) => {
            ...
            const splitValue = doc.splitTextToSize(value, pageWidth - margin * 2 - 70);
            doc.text(splitValue, margin + 70, yPos);
        ...
        const addCheckboxGroup = (label: string, options: {key: keyof typeof formState, label: string}[], otherKey?: keyof typeof formState, otherTextKey?: keyof typeof formState) => {
            ...
            doc.setFont('helvetica', 'normal');
            options.forEach(opt => {
                if (formState[opt.key]) {
                    doc.text(`- ${opt.label}`, margin + 5, yPos);
                    yPos += 5;
                }
            });
            if (otherKey && otherTextKey && formState[otherKey]) {
                 doc.text(`- Other: ${formState[otherTextKey] || ''}`, margin + 5, yPos);
                 yPos += 5;
            }
            yPos += 2;
        };
        ...
        const addRadioGroup = (label: string, name: keyof typeof formState, options: {value: string, label: string}[]) => {
            const selectedOption = options.find(o => o.value === formState[name]);
            if (selectedOption) {
                addField(label, selectedOption.label);
            }
        }
        ...
        addCheckboxGroup('Project Reqt.', [
            { key: 'reqArchitectural', label: 'Architectural Designing' },
            { key: 'reqInterior', label: 'Interior Decoration' },
            { key: 'reqLandscaping', label: 'Landscaping' },
            { key: 'reqTurnkey', label: 'Turnkey' },
